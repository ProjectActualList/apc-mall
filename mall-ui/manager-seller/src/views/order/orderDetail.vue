      ...
      <el-collapse  class="order-collapse" :value="['order', 'promotions', 'other']">
        ...
        <el-collapse-item title="其他信息（发票、备注）" name="other">
          ...
          <div class="order-item" v-if="orderDetail.cancel_reason">
            ...
            <span class="item-value">{{ orderDetail.cancel_reason }}</span>
          </div>
          <div class="order-item">
    ...
    methods: {
      ...
      GET_OrderDetail() {
        ...
        API_order.getOrderDetail(this.sn).then(response => {
          ...
          // 步骤条信息
          this.getStepList()
          // 是否可发货 如果可发货则获取物流公司信息列表 是否可发货 在线支付（已付款状态|拼团订单待发货状态可发货） 货到付款（已确认状态可发货）
          if ((this.orderDetail.payment_type === 'ONLINE' && (this.orderDetail.order_status === 'FORMED' || this.orderDetail.order_status === 'PAID_OFF')) ||
