    ...
    <van-popup v-model="showPopup" position="bottom" style="height:70%">
      <div class="spec-headr">
        <div class="pic-header">
          <img :src="selectedSpecImg.thumbnail || selectedSku.thumbnail">
        </div>
      ...
      <div class="spec-body">
        ...
        <div class="spec-num">
          ...
          <div class="quantity-wrapper">
            ...
            <input type="number" v-model="buyNum" class="quantity number" readonly>
      ...
      <div class="spec-footer">
        <a class="buy-btn add-cart" @click="$emit('to-assemble-buy-now')">确定</a>
      </div>
    </van-popup>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    mounted() {
      this.goodsId && this.GET_AssembleSkuList()
    },
    methods: {
      // 获取此商品参与拼团的sku规格列表
      GET_AssembleSkuList() {
        API_Promotions.getAssembleSkuList(this.goodsId).then(response => {
          const specList = []
      ...
      canSelected(specIndex, specList, spec_val) {
        if (specIndex) {
          const spec_id = specList[--specIndex].valueList.filter(key => key.selected)[0].spec_value_id
          const _ids = []
